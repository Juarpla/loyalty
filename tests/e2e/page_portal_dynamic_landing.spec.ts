...
test.describe("Dynamic Client Landing Page — /portal/[companyId]", () => {
  ...
  test("R1, R2, R3, R5: Correct page title, fetch settings, customized welcome message, SSID & no overflow", async ({
    ...
    // Intercept the company wifi settings API
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            ssid: "SuperCoffee_WiFi",
            wifi_password: "supersecurepass",
            welcome_title: "Welcome to SuperCoffee!",
            welcome_message: "Sign in to get premium speed Internet",
            brand_color: "#ea580c",
          },
        }),
      });
    ...
    // R3 & R5: Custom welcome copy and SSID badge visible
    ...
    await expect(cta).toContainText("Welcome to SuperCoffee!");
    await expect(cta).toContainText("Sign in to get premium speed Internet");
    await expect(ssidBadge).toContainText("SuperCoffee_WiFi");
  ...
  test("R4: Failed settings endpoint renders fallback portal using default credentials", async ({
    ...
    // Intercept settings request with error status
    await page.route(SETTINGS_API, async (route) => {
      await route.fulfill({
        status: 500,
        contentType: "application/json",
  ...
  test("R6: Successful registration updates page with WiFi QR rendering dynamic credentials", async ({
    ...
    // Intercept portal register route
    ...
    const ssidDisplay = page.locator("text=SuperCoffee_WiFi");
